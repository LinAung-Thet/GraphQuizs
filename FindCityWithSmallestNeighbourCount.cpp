...
class Solution {
public:
    void findReachableNeighbour(int& n, int& node, int& threshold, vector<vector<pair<int,int>>>& matrix, 
        vector<bool>& visited, vector<set<int>>& reachableCity, vector<vector<int>>& distMatrix) {
        if(node == n-1) return; // If we reach the last node, no need to proceed further

        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<>> pq; // [dist, node]
        vector<int> minDist(n, INT_MAX);

        pq.emplace(0, node); // Start from the node with distance 0;
        minDist[node] = 0;

        while(!pq.empty()){
            auto [dist, currNode] = pq.top(); pq.pop();

            for(auto& [w,t]: matrix[currNode]) {
                // if(visited[t]) {
                //     for(int i = 0; i < n; i++) {
                //         if(distMatrix[t][i] != -1 && distMatrix[t][i] <= threshold) {
                //             int accuW = distMatrix[t][i] + w;
                //             if(accuW <= threshold) {
                //                 distMatrix[node][i] = accuW;
                //                 distMatrix[i][node] = accuW; // Update the reverse distance

                //                 reachableCity[node].emplace(i);
                //                 reachableCity[i].emplace(node); // Add the reverse connection
                //             }
                //         }
                //     }
                //     continue;
                ...
                int accuW = dist + w;
                if(accuW > threshold) continue;
                if(accuW > minDist[t]) continue;

                minDist[t] = accuW;
                pq.emplace(accuW, t);
                reachableCity[node].emplace(t);
                reachableCity[t].emplace(node); // Add the reverse connection

                distMatrix[node][t] = accuW; // Update the distance matrix
                distMatrix[t][node] = accuW; // Update the reverse distance
            }
        }
    }
    int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) {
        // Build the adjacency matrix [dist, city]
        vector<vector<pair<int,int>>> matrix(n);
        for(auto& edge: edges) {
            int f = edge[0], t = edge[1], w = edge[2];
            matrix[f].emplace_back(w,t);
            matrix[t].emplace_back(w,f);
        }

        vector<bool> visited(n, false);
        vector<set<int>> reachableCity(n);
        ...
        int count = 0;
        for(int i=0; i<n; i++) {
            // Find the reachable neighbours for each city
            findReachableNeighbour(n, i, distanceThreshold, matrix, visited, reachableCity, dist);

            // Update the visited status
            visited[i] = true;
            int numNei = reachableCity[i].size();
            if(numNei < minNeighbour || numNei == minNeighbour) {
                if(numNei < minNeighbour) {
                    minNeighbour = numNei;
                    count = 0;
                }
                minNode = i;
